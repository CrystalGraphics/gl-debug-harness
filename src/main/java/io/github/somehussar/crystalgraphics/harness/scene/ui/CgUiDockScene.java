...
public class CgUiDockScene implements InteractiveSceneLifecycle, CgSystemInput.Keyboard, CgSystemInput.Mouse {
    ...
    public void init(HarnessContext ctx) {
        ...
        // THE REAL SHADER GRAPH. Re-adopted after the internal-stamping fix; the gallery deliberately
        // still builds its own inline, so one scene remains usable if this regresses.
        ...
        // THE NODE GRAPH THEME, and it is not optional decoration: a wire reads its colour out of the
        // cascade (NodePort.typeColor returns the dot.s computed border-color), so without this sheet the
        // nodes are grey boxes and every wire is colourless. Before ore, matching the gallery -- sheet
        // order is registration order and the later one wins.
        uiWindow.getStyleEngine().addStylesheet(StyleSheetRegistry.of("crystalgui:graph"));
